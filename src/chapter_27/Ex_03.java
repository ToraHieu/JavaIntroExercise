...
public class Ex_03 {
    public static void main(String[] args) {
        // Create a map
        MyMap<String, Integer> map = new Ex03HashMap<>();
        map.put("Smith", 30);
        map.put("Anderson", 31);
        map.put("Lewis", 29);
        map.put("Cook", 29);
        ...
        MyMap<Integer, Integer> myMap = new Ex03HashMap<>(11, 0.75f);
        myMap.put(45, 1);
        myMap.put(58, 2);
        myMap.put(4, 3);
        myMap.put(28, 4);
...
class Ex03HashMap<K, V> implements MyMap<K, V> {
     ...
     * default load factor
     */
    public Ex03HashMap(int initialCapacity) {
     ...
     * and load factor
     */
    public Ex03HashMap(int initialCapacity, float loadFactorThreshold) {
        this.capacity = Math.min(initialCapacity, MAXIMUM_CAPACITY);

        this.loadFactorThreshold = loadFactorThreshold;
        //noinspection unchecked
        table = new Ex03Entry[capacity];
    }
    ...
    public java.util.Set<MyMap.Entry<K, V>> entrySet() {
        ...
        for (int i = 0; i < capacity; i++) {
            if (table[i] != null && table[i].state.equals(Ex03Entry.State.OCCUPIED)) {
                set.add(table[i]);
            }
        }
    ...
    public V put(K key, V value) {
        ...
        if (table[availableIndex] != null) {
            ...
            table[availableIndex].state = Ex03Entry.State.OCCUPIED;
        }
        else {
            table[availableIndex] = new Ex03Entry<>(key, value);
        }
        size++; // Increase size
    ...
    private int probeAvailableIndex(K key) {
        int k = hash(key.hashCode()), h = secondaryHash(key.hashCode()), i = 1,
                probeIndex = k % capacity;
        while (table[probeIndex] != null) {
            if (table[probeIndex].state.equals(Ex03Entry.State.MARKED))
    ...
    public void remove(K key) {
        ...
        if (index != -1) {
            table[index].state = Ex03Entry.State.MARKED;
            size--;
        }
    }
    ...
    private void rehash() {
        ...
        //noinspection unchecked
        table = new Ex03Entry[capacity]; // Create a new hash table
        size = 0; // Reset size to 0

        for (Entry<K, V> entry : set) {
            if (((Ex03Entry<K, V>) entry).state.equals(Ex03Entry.State.OCCUPIED))
    ...
    public static class Ex03Entry<K, V> extends Entry<K, V> {
        enum State {OCCUPIED, MARKED}

        private State state;

        public Ex03Entry(K key, V value) {
            super(key, value);
