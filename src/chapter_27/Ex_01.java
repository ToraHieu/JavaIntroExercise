...
public class Ex_01 {
    public static void main(String[] args) {
        // Create a map
        MyMap<String, Integer> map = new Ex01HashMap<>();
        map.put("Smith", 30);
        map.put("Anderson", 31);
        map.put("Lewis", 29);
        map.put("Cook", 29);
        ...
        MyMap<Integer, Integer> myMap = new Ex01HashMap<>(11);
        myMap.put(44, 1);
        myMap.put(4, 2);
        myMap.put(16, 3);
        myMap.put(28, 4);
...
class Ex01HashMap<K, V> implements MyMap<K, V> {
     ...
     * default load factor
     */
    public Ex01HashMap(int initialCapacity) {
     ...
     * and load factor
     */
    public Ex01HashMap(int initialCapacity, float loadFactorThreshold) {
        this.capacity = Math.min(initialCapacity, MAXIMUM_CAPACITY);

        this.loadFactorThreshold = loadFactorThreshold;
        //noinspection unchecked
        table = new Ex01Entry[capacity];
    }
    ...
    public java.util.Set<MyMap.Entry<K, V>> entrySet() {
        ...
        for (int i = 0; i < capacity; i++) {
            if (table[i] != null && table[i].state.equals(Ex01Entry.State.OCCUPIED)) {
                set.add(table[i]);
            }
        }
    ...
    public V put(K key, V value) {
        ...
        if (table[availableIndex] == null) {
            table[availableIndex] = new Ex01Entry<>(key, value);
        } else {
            table[availableIndex].key = key;
            table[availableIndex].value = value;
            table[availableIndex].state = Ex01Entry.State.OCCUPIED;
        }
        size++; // Increase size
    ...
    private int probeIndex(K key) {
        ...
        while (table[probeIndex] != null) {
            if (table[probeIndex].getKey().equals(key)) {
                if (table[probeIndex].state.equals(Ex01Entry.State.OCCUPIED))
    ...
    private int probeAvailableIndex(K key) {
        int k = hash(key.hashCode()), i = 1, probeIndex = k % capacity;
        while (table[probeIndex] != null) {
            if (table[probeIndex].state.equals(Ex01Entry.State.MARKED))
    ...
    public void remove(K key) {
        ...
        if (index != -1) {
            table[index].state = Ex01Entry.State.MARKED;
            size--;
        }
    }
    ...
    private void rehash() {
        ...
        //noinspection unchecked
        table = new Ex01Entry[capacity]; // Create a new hash table
        size = 0; // Reset size to 0

        for (Entry<K, V> entry : set) {
             if (((Ex01Entry<K, V>) entry).state.equals(Ex01Entry.State.OCCUPIED))
    ...
    public static class Ex01Entry<K, V> extends Entry<K, V> {
        enum State {OCCUPIED, MARKED}

        private State state;

        public Ex01Entry(K key, V value) {
            super(key, value);
